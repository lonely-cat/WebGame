...
async function main() {
  ...
  try {
    ...
    await pageA.click('[data-row="0"][data-col="1"]');

    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.latestMove?.piece === "red-cannon"
        && state.latestMove?.captured === "black-horse"
        && state.latestMove?.toRow === 0
        && state.latestMove?.toCol === 1;
    }, null, { timeout: 10000 });
