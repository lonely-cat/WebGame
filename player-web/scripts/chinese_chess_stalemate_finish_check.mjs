...
async function main() {
  ...
  try {
    ...
    await pageA.evaluate(() => window.load_chinese_chess_scenario("stalemate_ready_red"));
    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.scenarioName === "stalemate_ready_red" && state.winner === null;
    }, null, { timeout: 10000 });
    ...
    await pageA.waitForFunction(() => {
      ...
      return state.endReason === "stalemate"
        && state.winner === "red"
        && state.latestMove?.piece === "red-cannon"
