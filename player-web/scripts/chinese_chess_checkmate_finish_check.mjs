...
async function main() {
  ...
  try {
    ...
    await pageA.evaluate(() => window.load_chinese_chess_scenario("checkmate_ready_red"));
    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.scenarioName === "checkmate_ready_red" && state.winner === null;
    }, null, { timeout: 10000 });

    await pageA.click('[data-row="2"][data-col="4"]');
    await pageA.click('[data-row="1"][data-col="4"]');

    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.endReason === "checkmate"
        && state.winner === "red"
        && state.latestMove?.piece === "red-rook"
        && state.latestMove?.toRow === 1
        && state.latestMove?.toCol === 4;
    }, null, { timeout: 10000 });
