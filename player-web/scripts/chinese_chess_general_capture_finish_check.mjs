...
async function main() {
  ...
  try {
    ...
    await pageA.evaluate(() => window.load_chinese_chess_scenario("capture_general_ready_red"));
    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.scenarioName === "capture_general_ready_red" && state.winner === null;
    }, null, { timeout: 10000 });

    await pageA.click('[data-row="2"][data-col="4"]');
    await pageA.click('[data-row="0"][data-col="4"]');

    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.endReason === "captured_general"
        && state.winner === "red"
        && state.latestMove?.piece === "red-rook"
        && state.latestMove?.captured === "black-general";
    }, null, { timeout: 10000 });
