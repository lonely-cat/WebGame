...
async function main() {
  ...
  try {
    ...
    await pageA.click('[data-row="9"][data-col="1"]');
    await pageA.click('[data-row="7"][data-col="2"]');

    await pageA.waitForFunction(() => {
      const state = JSON.parse(window.render_game_to_text());
      return state.latestMove?.piece === "red-horse" && state.latestMove?.toRow === 7 && state.latestMove?.toCol === 2;
    }, null, { timeout: 10000 });
