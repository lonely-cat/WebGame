...
async function main() {
  ...
  try {
    ...
    await pageA.waitForTimeout(47000);

    const timedOutA = JSON.parse(await pageA.evaluate(() => window.render_game_to_text()));
    const timedOutB = JSON.parse(await pageB.evaluate(() => window.render_game_to_text()));
